...
func main() {
	...
	providedNoUiPlaylist := flag.String("noui-playlist", "", "Select just one Playlist to play when no User Interface is used.")
	flag.Parse()
	...
	if *providedUi {
		...
		err := ui.StartNoUserInterface(events, providedNoUiSilent, providedNoUiPlaylist)
		if err != nil {
			fmt.Println(err)
		}
	}
